...
crossing(x = 1:100, y = 1:100) |>
  mutate(x2 = sapply(x, is_left_truncatable),
         y2 = sapply(y, is_right_truncatable),
         class = case_when(!x2 & !y2 ~ "neither special",
                           x2 & !y2 ~ "left special",
                           !x2 & y2 ~ "right special",
                           x2 & y2 ~ "double special")) |>
  filter(class != "neither special") |>
  ggplot(aes(x, y)) +
  geom_point(aes(col = class), show.legend = F) +
  scale_color_manual(values = c("black", "#C9D9C6", "#D6C6D9")) +
